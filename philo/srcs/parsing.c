...
static void     convert_to_ms(t_table *table, long time_to_die, long time_to_eat, long time_to_sleep)
{
    if (time_to_die > LONG_MAX / 1000 || time_to_eat > LONG_MAX / 1000 || time_to_sleep > LONG_MAX / 1000)
...
static void    check_numbers(t_table *table, char **argv)
{
    ...
    //     error_exit("All arguments must be positive numbers");
